...
#define ADC_ENTER_CRITICAL() do { adc_context.spinlock = irq_lock(); } while(0) // portENTER_CRITICAL(&rtc_spinlock)
#define ADC_EXIT_CRITICAL() do { irq_unlock(adc_context.spinlock); } while(0) //  portEXIT_CRITICAL(&rtc_spinlock)
...
#define SAR_ADC1_LOCK_ACQUIRE() do { k_mutex_lock(&adc_context.adc1_lock, K_FOREVER); } while(0)  // _lock_acquire(&sar_adc1_lock)
#define SAR_ADC1_LOCK_RELEASE() do { k_mutex_unlock(&adc1_lock); } while(0) //  _lock_release(&sar_adc1_lock)
//static _lock_t sar_adc2_lock;
#define SAR_ADC2_LOCK_ACQUIRE() do { k_mutex_lock(&adc2_lock, K_FOREVER); } while(0) //   _lock_acquire(&sar_adc2_lock)
#define SAR_ADC2_LOCK_RELEASE() do { k_mutex_unlock(&adc2_lock); } while (0) // _lock_release(&sar_adc2_lock)
//portMUX_TYPE adc_reg_lock = portMUX_INITIALIZER_UNLOCKED;
#define ADC_REG_LOCK_ENTER() do { k_mutex_lock(&reg_lock, K_FOREVER); } while(0) //       portENTER_CRITICAL(&adc_reg_lock)
#define ADC_REG_LOCK_EXIT() do { k_mutex_unlock(&reg_lock); } while(0) //         portEXIT_CRITICAL(&adc_reg_lock)
...
esp_err_t adc_digi_initialize(const adc_digi_init_config_t *init_config)
{
	...
	s_adc_digi_ctx->buf = malloc(init_config->max_store_buf_size);
    ring_buf_init(&s_adc_digi_ctx->ringbuf_hdl, init_config->max_store_buf_size, s_adc_digi_ctx->buf);
...
#if 0// CONFIG_IDF_TARGET_ESP32C3
/*---------------------------------------------------------------
